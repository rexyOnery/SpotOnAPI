...
namespace WebApi.Services
{
    ...
    public class AccountService : IAccountService
    {
        ...
        public bool Register(RegisterRequest model, string origin)
        {
            ...
            VerifyEmail(account.VerificationToken);
            return true;
        }
        ...
        public string ForgotPassword(ForgotPasswordRequest model, string origin)
        {
            ...
            if (account.Question != model.Question && account.Answer != model.Answer) return "false";
        ...
        public IEnumerable<AccountResponse> GetAllAgents()
        {
            var accounts = _context.Accounts.Where(c => c.Role == Role.User);
            return _mapper.Map<IList<AccountResponse>>(accounts);
        }
        ...
        public void SendMail(string name, string phone, string subject, string message, string email)
        {
            _emailService.Send(
               to: "contact@steezz24.com",
               subject: subject,
               html: $@"<h4>Name: {name}.<br />Phone: {phone}<br />Email: {email}</h4><br /><br />
                         {message}",
               from: email
           );
        }
        ...
        {
            var account = _context.Accounts.FirstOrDefault(c => c.Email == email);
            if (account == null) throw new KeyNotFoundException("Account not found");
            return account;
        }
        ...
        private void sendVerificationEmail(Account account, string origin)
        {
            ...
            _emailService.Send(
                to: account.Email,
                subject: "Steezz Laundry Sign-up Verification - Verify Email",
        ...
        private void sendAlreadyRegisteredEmail(string email, string origin)
        {
            ...
            _emailService.Send(
                to: email,
                subject: "Steezz Laundry Sign-up Verification - Email Already Registered",
